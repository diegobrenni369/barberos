...
export function AppointmentDragProvider({ children, date, appointments }: { children: ReactNode; date: string; appointments: AppointmentData[] }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [message, setMessage] = useState("");
  const lock = useRef(false);
  return <DragPending.Provider value={pending}><DragDropProvider sensors={sensors}
    ...
    onDragEnd={event => {
      ...
      if (!target || target.data.valid !== true) { setMessage("No disponible en ese horario. La reserva no se movió."); return; }
  ...
  </DragDropProvider></DragPending.Provider>;
}
...
export function AppointmentDropSlot({ barberId, minute, gridStart, gridEnd, intervals, breaks, blocks, appointments }: {
  ...
  const valid = end <= gridEnd && intervals.some(item => item.startMinute <= start && item.endMinute >= end)
  ...
  const { ref, isDropTarget } = useDroppable({ id: `slot-${barberId}-${start}`, collisionDetector: pointerIntersection, data: { barberId, minute: start, valid } });
