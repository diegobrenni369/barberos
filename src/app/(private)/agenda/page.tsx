...
export default async function AgendaPage({ searchParams }: { searchParams: Promise<{ date?: string; error?: string; cancelled?: string }> }) {
  ...
  const [businessHour, barbers, customers, services, appointments, blocks, reasons] = await Promise.all([
    ...
    prisma.service.findMany({ where: { barbershopId: membership.barbershopId, isActive: true }, orderBy: { name: "asc" }, select: { id: true, name: true, durationMinutes: true, price: true, barberServices: { where: { barbershopId: membership.barbershopId, barber: { isActive: true } }, select: { barberId: true } } } }),
    prisma.appointment.findMany({ where: { barbershopId: membership.barbershopId, startsAt: { gte: range.start, lt: range.end }, ...(showCancelled ? {} : { status: { not: "CANCELLED" } }) }, include: { customer: { select: { name: true } }, service: { select: { name: true } }, sale: { select: { total: true, currency: true, status: true, payments: { select: { method: true, amount: true }, orderBy: { createdAt: "asc" } } } } }, orderBy: { startsAt: "asc" } }),
