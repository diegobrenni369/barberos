...
export default async function AgendaPage({ searchParams }: { searchParams: Promise<{ date?: string; error?: string; cancelled?: string }> }) {
  ...
  return <section className="space-y-6"><PageHeader title="Agenda" description={titleDate.charAt(0).toUpperCase() + titleDate.slice(1)} />{params.error && <p className="rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{params.error}</p>}{!businessHour || businessHour.isClosed ? <ClosedAgenda date={date} today={todayParts.date} /> : <AgendaView date={date} today={todayParts.date} showCancelled={showCancelled} nowMinutes={nowMinutes} startMinute={businessHour.opensMinute} endMinute={businessHour.closesMinute} barbers={barbers.map(({ id, name }) => ({ id, name }))} customers={customers} services={services.map((item) => ({ ...item, price: item.price.toString() }))} appointments={serialized} availability={availability} recurringBreaks={recurringBreaks} blocks={serializedBlocks} reasons={reasons} />}</section>;
}
