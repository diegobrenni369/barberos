...
export default async function BookingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [services, barbers, hours] = await Promise.all([
    prisma.service.findMany({ where: { barbershopId: shop.id, isActive: true, isOnlineBookingEnabled: true, onlinePaymentPolicy: "NONE", barberServices: { some: { barbershopId: shop.id, barber: { isActive: true } } } }, select: { id: true, name: true, durationMinutes: true, price: true, barberServices: { where: { barbershopId: shop.id, barber: { isActive: true } }, select: { barberId: true } } }, orderBy: { name: "asc" } }),
